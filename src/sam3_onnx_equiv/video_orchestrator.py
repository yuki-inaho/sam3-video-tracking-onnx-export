...
class PythonMemoryBank:
    ...
    def __init__(self) -> None:
        self.cond_frame_outputs: dict[int, dict[str, Any]] = {}
        self.non_cond_frame_outputs: dict[int, dict[str, Any]] = {}

    def store_cond(self, frame_idx: int, entry: dict[str, Any]) -> None:
        self.cond_frame_outputs[frame_idx] = entry

    def store_non_cond(self, frame_idx: int, entry: dict[str, Any]) -> None:
...
def _preprocess_frame(pil_img: Image.Image) -> np.ndarray:
...
def _upsample_bilinear(mask: np.ndarray, target_h: int, target_w: int) -> np.ndarray:
...
def _to_bf16_round(arr: np.ndarray) -> np.ndarray:
...
def _get_1d_sine_pe(pos_inds: np.ndarray, dim: int, temperature: float = ROPE_THETA) -> np.ndarray:
...
def _obj_ptr_tpos(rel_pos_list: list[int], max_abs_pos: int, constants: Constants) -> np.ndarray:
    ...
    return (pos_enc @ constants.obj_ptr_tpos_proj_weight.T
            + constants.obj_ptr_tpos_proj_bias).astype(np.float32)   # (N, 64)
...
def _build_memory_prompt(
    ...
    tpos_enc = constants.maskmem_tpos_enc   # (7,1,1,64)

    maskmem_feats: list[np.ndarray] = []    # each (5184,1,64)
    maskmem_poses: list[np.ndarray] = []    # each (5184,1,64)

    # --- (1) selected conditioning frames (t=0). Only past cond frames. ---
    # select_closest_cond_frames returns ALL cond frames when len <= max_cond.
    sel_cond_keys = sorted(cond.keys())
    for t_cond in sel_cond_keys:
        out = cond[t_cond]
        idx = NUM_MASKMEM - 0 - 1   # = 6 (t = 0 for selected cond frame)
        f = out["maskmem_features"][0].reshape(MEM_DIM, HW).T[:, None, :]   # (5184,1,64)
        p = out["maskmem_pos_enc"][-1][0].reshape(MEM_DIM, HW).T[:, None, :]
        tpe = tpos_enc[idx]   # (1,1,64)
        # tpos goes to the key (prompt_pos) only, never the value (prompt):
        # tracker_base.py:659 prompt = feats only; :676-678 prompt_pos = pos + tpos.
        maskmem_feats.append(f)
        maskmem_poses.append(p + tpe)

    # --- (2) non-conditioning frames t_pos = 1..num_maskmem-1 ---
    for t_pos in range(1, NUM_MASKMEM):
        t_rel = NUM_MASKMEM - t_pos                 # frames before current
        ...
        t = t_pos
        idx = NUM_MASKMEM - t - 1
        f = out["maskmem_features"][0].reshape(MEM_DIM, HW).T[:, None, :]
        p = out["maskmem_pos_enc"][-1][0].reshape(MEM_DIM, HW).T[:, None, :]
        tpe = tpos_enc[idx]
        # tpos goes to the key (prompt_pos) only, never the value (prompt):
        # tracker_base.py:659 prompt = feats only; :676-678 prompt_pos = pos + tpos.
    ...
    # selected cond frames (t <= frame_idx)
    for t_cond in sel_cond_keys:
        if t_cond <= frame_idx:
            pos_and_ptrs.append((frame_idx - t_cond, cond[t_cond]["obj_ptr"]))
    # up to (max_obj_ptrs - 1) non-cond frames before current frame
    for t_diff in range(1, max_obj_ptrs):
        t = frame_idx - t_diff
        if t < 0:
            break
        out = non_cond.get(t, None)
        if out is not None:
    ...
    num_obj_ptr_tokens = int(obj_tokens.shape[0])
...
class VideoOrchestrator:
    ...
    def run_clip(
        ...
        frame0_point_coords_norm: np.ndarray,
        frame0_point_labels: np.ndarray,
        ...
        for frame_idx, pil_frame in enumerate(frames_pil):
            ...
            high_res_feat0 = _conv1x1(backbone_fpn_0, C.conv_s0_weight, C.conv_s0_bias)
            high_res_feat1 = _conv1x1(backbone_fpn_1, C.conv_s1_weight, C.conv_s1_bias)
            ...
            if not use_mem_this_frame:
                ...
                pix_feat_seq = fpn2_seq + C.no_mem_embed              # (5184,1,256)
                pix_feat_with_mem = pix_feat_seq.transpose(1, 2, 0).reshape(
                    1, D_MODEL, FEAT_H, FEAT_W
                )
            else:
                prompt, prompt_pos, n_obj_tokens = _build_memory_prompt(
                    frame_idx, num_frames, bank, C
                )
                ...
                pix_feat_with_mem = memory_seq.transpose(1, 2, 0).reshape(
                    1, D_MODEL, FEAT_H, FEAT_W
                )
            ...
            # --- Step 5: high-res mask + mask_for_mem (binary vs sigmoid) - #
            ...
            # --- Step 7: add no_obj_embed_spatial (_encode_new_memory:845-848) #
            ...
            log.info(
                ...
                frame_idx, obj_score, int(binary_mask.sum()), use_mem_this_frame,
            )
